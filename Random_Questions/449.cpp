...
public:
    ...
    TreeNode *deserialize(string data)
    {
        ...
        function<TreeNode *(int, int)> build = [&](int lower,
                                                   int upper) -> TreeNode *
        {
            if (index == vals.size())
                return nullptr;
            ...
            // Current value doesn't belong to this range
            ...
            root->left = build(lower, val - 1);

            // Right subtree: values > root->val
            root->right = build(val + 1, upper);

            return root;
        };
        return build(0, 10000);
    }
};
